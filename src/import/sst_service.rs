...
impl<E: Engine> ImportSst for ImportSstService<E> {
    ...
    fn check_and_compact(&mut self, _ctx: RpcContext, req: CheckAndCompactRequest, sink: UnarySink<CheckAndCompactRespose>) {
        let label = "check_and_compact";
        let timer = Instant::now_coarse();
        ...
        let handle_task = async move {
            ...
            opts.set_bottom_level_range_overlap_check(true);
            for cf in [CF_WRITE, CF_DEFAULT] {
                if let Err(e) = engine.compact_range_cf(cf, Some(&start), Some(&end), opts.clone()) {
                    warn!("manual compact range failed"; "cf" => cf, "start" => ?log_wrappers::Value::key(req.get_range().get_start()),
                        "end" => ?log_wrappers::Value::key(req.get_range().get_end()), "err" => ?e);
                }
            }
            ...
            let resp = CheckAndCompactRespose::default();
            send_rpc_response!(Ok(resp), sink, label, timer);
        };

        self.threads.spawn(handle_task);
    }
    fn remove_force_partition_range(&mut self, ctx: RpcContext, req: RemoveRangeRequest, sink: UnarySink<RemoveRangeResponse>) {
        let label = "check_and_compact";
        let timer = Instant::now_coarse();
        
        let start = keys::data_key(req.get_range().get_start());
        let end = keys::data_end_key(req.get_range().get_end());
        ...
        let resp = RemoveRangeResponse::default();
        ctx.spawn(async move { send_rpc_response!(Ok(resp), sink, label, timer) });
    }
}
