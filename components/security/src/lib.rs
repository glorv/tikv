...
use grpcio::{
    CertificateRequestType, Channel, ChannelBuilder, ChannelCredentialsBuilder, CheckResult, RpcContext, RpcStatus, RpcStatusCode, ServerBuilder, ServerChecker, ServerCredentials, ServerCredentialsBuilder, ServerCredentialsFetcher,
    Service,
};
...
impl SecurityManager {
    ...
    pub fn bind(&self, mut sb: ServerBuilder, addr: &str, port: u16) -> ServerBuilderWithAddrs {
        ...
        let creds = if self.cfg.ca_path.is_empty() {
            ...
            ServerCredentials::with_fetcher(fetcher, CertificateRequestType::RequestAndRequireClientCertificateAndVerify)
        };
